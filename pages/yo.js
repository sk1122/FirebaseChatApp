	...
	return (
		...
		<div className='flex flex-col justify-center items-center'>
			...
			<div className="w-full h-full grid grid-cols-3 grid-rows-2 gap-10 m-10">
				{users.map((value) => (
					<div className="flex flex-col justify-center items-center cursor-pointer" onClick={() => submit(value.uid)}>
						<img class="w-10" src={value.photoURL} alt="Sunset in the mountains" />
						<div class="px-6 py-4">
							<div class="font-bold text-xl mb-2">{value.displayName}</div>
							<p class="text-gray-700 text-base">
								{value.email}
							</p>
						</div>
						<div class="px-6 pt-4 pb-2">
							<span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{value.online ? "Online" : "Offline"}</span>
						</div>
					</div>
				))}
